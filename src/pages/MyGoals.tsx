  ...
  const userGoals = useMemo(() => {
    ...
    return goals
      ...
      .map((g: any) => {
        ...
        // Busca a contagem de atividades para este usuário e tipo de tarefa
        const act = activities.find(
          (a) => a.user_id === profile.id && a.action_type === tt?.name
        );

        return {
          label: tt?.description || tt?.name || "Tarefa",
          current: act?.count || 0,
          // MUDANÇA AQUI: Agora usamos o period_goal calculado dinamicamente no hook
